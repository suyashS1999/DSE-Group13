...
#%% ------------------------ INPUTS ------------------------
MTOW = 78000*g;							# Max Take off Weight [N]
MLW = 0.86*MTOW									# Max Landing Weight [N]
OEW = 42600*g;							# Operational empty weight [N]
Payload_max = 19.9*1000*g;						# Payload weight [N]
Max_Fuel_cap = 23859*0.785*g;						# Max fuel weight [N]
cj = 1.48e-05;									# Specific fuel consumption [kg/Ns]
L_D = 16.3;										# Lift to drag ratio [-]
...
Cl_max = array([2.3, 2.5, 3.0]);				# Cl max values for assessment [-]
Cl_max_takeoff = array([1.0, 1.9]);		# Cl_max for take off [-]
A = array([8.0, 10.0]);						# Aspect ratio [-]
...
pre_cruise_fuel_frac = 0.2;					# Pre cruise fuel fraction [-]
post_cruise_fuel_frac = 0.09;					# Post cruise fuel fraction [-]
#%% ------------------------ Main ------------------------
