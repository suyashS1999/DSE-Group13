...
Ct_h = taper_h*Cr_h
sweep_half_h = tl.sweep_x(0.5,np.radians(sweep_le_h),Cr_h,Ct_h,b_h)
CL_h_alpha = tl.CL_alpha_DATCOM(A_h,M_cruise,a0_h,sweep_half_h)
M_crit_h = M_crit_a_h/np.cos(np.radians(sweep_le_h))
...
Ct_v = taper_v*Cr_v
sweep_half_v = tl.sweep_x(0.5,np.radians(sweep_le_v),Cr_v,Ct_v,b_v)
CL_v_alpha = tl.CL_alpha_DATCOM(A_v,M_cruise,a0_v,sweep_half_v)
M_crit_v = M_crit_a_v/np.cos(np.radians(sweep_le_v))
