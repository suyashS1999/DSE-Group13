...
As = 74.74 #Area of the strut
bs = 38.2 #Span of the strut
...
r = 800                           #distance from source to observer
...
for i in range(len(thetal)):
    ...
    G_strut = geometry_func(As,bs,rho_inf,M,c,dynamic_visc,df,n_main,d_main,"Clean wing")
    ...
    L_strut = length_scale(G_strut,bs,As,d_main,"Clean wing")
    ...
    K_strut = constants_Ka("Clean wing",n_main)[0]
    a_strut = constants_Ka("Clean wing",n_main)[1]
    ...
    P_strut = powers(K_strut, M, a_strut, G_strut,rho_inf,c,bs)
    ...
    S_strut = Strouhal_number(f,L_strut,M,theta,c)
    ...
    F_strut = dimensionless_empirical_spectral_func(S_strut,n_main,"Clean wing")
    ...
    D_strut = directivity(theta,phi,df,"Clean wing")
    ...
    pe_sq_strut = pe_squared(rho_inf,c,P_strut,D_strut,F_strut,r,M,theta)
    ...
    pe_sq[:,i] = (pe_sq_cleanwing + pe_sq_strut + pe_sq_slats + pe_sq_flaps + 2*pe_sq_mainlg + 3*pe_sq_noselg)[0]
