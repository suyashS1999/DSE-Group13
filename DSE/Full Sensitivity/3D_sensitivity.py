# -*- coding: utf-8 -*-
"""
Created on Fri Jun 26 23:11:48 2020
...
import math
from PlottingModule import Plot_2D as p2d
...
OEW_arr = np.linspace(0.9*OEW, 1.1*OEW, 30)


n = (len(OEW_arr), len(OEW_arr))

MF_arr  = np.zeros(n)
MTOW_arr = np.zeros(n)
LD_arr = np.linspace(0.9*LD_cruise, 1.1*LD_cruise, len(OEW_arr))
TSFC_arr = np.linspace(0.9*TSFC_cruise, 1.1*TSFC_cruise, len(OEW_arr))
range_arr = np.zeros(n)
efficiency_arr = np.zeros(n) #TSFC for rows, LD for columns

OEW_parr = (OEW_arr - OEW)/OEW*100


for l in range (len(OEW_arr)):
    
    TSFC_cruise = TSFC_arr[l]
    ...
    for k in range (len(OEW_arr)):
        ...
        efficiency_arr[l, k] = MF/3995/194
            
        MF_arr[l, k] = MF
        MTOW_arr[l, k] = MTOW
        
        total_range = 400 + range_C/1000    
        range_arr[l, k] = total_range
...
tsfc, ld = np.meshgrid(TSFC_parr, LD_parr)

fig= plt.figure()

p2d(tsfc, ld, efficiency_arr, "TSFC", "LD", "Fuel burn", figure = fig)

a320 = np.ones(n)*0.01431*0.9

p2d(tsfc, ld, a320, "", "",  "", figure = fig)
